...
public class HealVialSpell : ModItem
{
	...
	{
		// DisplayName.SetDefault("Lesser Healing Spell");
	...
	public override void SetDefaults()
	{
		...
		Item.value = 500;
		Item.shoot = Mod.Find<ModProjectile>("HealVialProj").Type;
		Item.shootSpeed = 8f;
		Item.autoReuse = true;
	}
	...
	public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
	{
		...
		{
			Projectile.NewProjectile((IEntitySource)source, position, velocity, Mod.Find<ModProjectile>("HealVialProj").Type, damage, knockback, player.whoAmI, 0f, 0f);
		}
		if (this.shot == 2)
		{
	...
	public override void ModifyTooltips(List<TooltipLine> tooltips)
	{
		...
		string sepText = "Splash heals players for " + (this.splashHealAmount + player.GetModPlayer<Global>().spellPower) + " health\nCreates a brief, lingering health cloud that lasts for 5 seconds and heals " + (1 + player.GetModPlayer<Global>().spellPower / 2) + " health";
