...
public class HoneyLinger : ModProjectile
{
	private int time;
	...
	public override void AI()
	{
		...
		for (int i = 0; i < 1; i++)
		{
			...
			if (Projectile.timeLeft == 802 && distance2 < (float)dist2)
			{
				int healingAmount2 = 15 + p.GetModPlayer<Global>().spellPower;
		...
		int dust3 = Dust.NewDust(Projectile.position, Projectile.width + 96 + xx, Projectile.height + yy, 152);
		Main.dust[dust3].noGravity = true;
		Main.dust[dust3].velocity *= 0f;
		Main.dust[dust3].scale *= 1.5f;
		int dust4 = Dust.NewDust(Projectile.position, Projectile.width + 96 + xx, Projectile.height + yy, 153);
		Main.dust[dust4].velocity *= 0f;
		int dust5 = Dust.NewDust(Projectile.position, Projectile.width + 96 + xx, Projectile.height + yy, 153);
