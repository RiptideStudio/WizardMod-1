...
public class GreatHealLinger : ModProjectile
{
	...
	public override void AI()
	{
		...
		for (int i = 0; i < 1; i++)
		{
			...
			if (Projectile.timeLeft == 402 && distance2 < (float)dist2)
			{
				int healingAmount2 = 25 + p.GetModPlayer<Global>().spellPower;
			...
			if (Projectile.timeLeft % 40 == 0)
			{
				...
				{
					int healingAmount = 3 + p.GetModPlayer<Global>().spellPower / 2;
