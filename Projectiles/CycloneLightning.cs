...
public class CycloneLightning : ModProjectile
{
	...
	public override void AI()
	{
		...
		if (target)
		{
			...
			Projectile.velocity = (10f * Projectile.velocity + move) / 1.5f;
			this.AdjustMagnitude(ref Projectile.velocity);
		}
		this.time++;
		...
		if (Main.rand.Next(1) == 0)
		{
			...
			Main.dust[dust].scale = (float)Main.rand.Next(135, 180) * 0.013f;
			Main.dust[dust].noGravity = true;
			int dust2 = Dust.NewDust(position, 1, 1, 160);
			Main.dust[dust2].velocity *= 0.7f;
			Main.dust[dust2].scale = (float)Main.rand.Next(135, 180) * 0.013f;
