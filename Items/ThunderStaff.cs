...
public class ThunderStaff : ModItem
{
	...
	public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
	{
		...
		Vector2 ShootOffset = new Vector2(position.X+player.direction*12f, position.Y-12f);
		Projectile.NewProjectile((IEntitySource)source, ShootOffset, velocity, Mod.Find<ModProjectile>("ThunderStaffProj").Type, damage, knockback, player.whoAmI, 2f, 2f);
		return false;
	}
